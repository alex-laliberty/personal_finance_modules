...
def add_transaction():
    transactions = load_transactions(print_message=False)
    new_transaction = {}
    ...
    with open("financial_transactions.csv", "a", newline="") as my_file:
        writer = csv.DictWriter(my_file, fieldnames=("transaction_id","date","customer_id","amount","type","description"))
...
def delete_transaction():
    ...
    while True:
        try:
            transaction_id = int(input(f"Select transaction to delete ({transactions[0]["transaction_id"]}-{transactions[-1]["transaction_id"]}): "))-1
            if transaction_id not in range(len(transactions)):
                raise IndexError
            print(f"Transaction ID: {transactions[transaction_id]["transaction_id"]}, Date: {transactions[transaction_id]["date"]}, Customer ID: {transactions[transaction_id]["customer_id"]}, Amount: {transactions[transaction_id]["amount"]}, Type: {transactions[transaction_id]["type"]}, Memo: {transactions[transaction_id]["description"]}")
...
def generate_report(filename="financial_summaries.txt"):
    transactions = load_transactions(print_message=False)
    todays_date = datetime.now()
